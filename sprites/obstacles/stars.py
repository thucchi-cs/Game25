import pygame
import constants
import sprites.text as text
import sprites.shell as shell

class Star(pygame.sprite.Sprite):
    def __init__(self, pos):
        super().__init__()
        self.image = pygame.image.load("graphics/stars.png")
        self.image = pygame.transform.scale(self.image, (25,25))
        self.rect = self.image.get_rect()
        self.rect.x = pos[0]
        self.rect.y = pos[1]
        self.speed = 1
        self.counter = 0
        self.pos = 0
        self.following = False
        self.path = []
        self.gate = None
    
    def scroll(self, addition):
        self.rect.y += constants.SPEED + addition
    
    def bounce(self):
        self.counter += 1
        self.rect.y += self.speed
        if self.counter % 7 == 0:
            self.speed *= -1
    
    def update(self):
        if self.following:
            self.animate()
    
    def set_path(self, path):
        self.path = path
        
    def animate(self):
        if self.pos < len(self.path):
            self.rect.centerx = self.path[self.pos][0]
            self.rect.centery = self.path[self.pos][1]
            self.pos += 1
        else:
            self.following = False
            constants.star_counter.counter += 1
            self.remove(constants.all)
            self.pos = 0
                
class StarCounter(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.image.load("graphics/stars.png")
        self.image = pygame.transform.scale(self.image, (30,30))
        self.rect = self.image.get_rect()
        self.rect.x = 351
        self.rect.y = 10
        self.counter = 0
        self.text = text.Text("fonts/COMIC.TTF", 20, str(self.counter), (255,255,255), 399, 23)
    
    def draw(self, screen):
        screen.blit(self.image, (self.rect.x, self.rect.y))
        self.text.text = str(self.counter)
        self.text.blit_text(screen)